...
var certificatesPath = common.extensionCertPath;

var ACTIVE_PEM_FILE = {
    'AUTHOR_KEY_FILE' : certificatesPath + path.sep + 'developer_active_cert/tizen_developer_private_key.pem',
    'AUTHOR_CERT_FILE' : certificatesPath + path.sep + 'developer_active_cert/tizen_developer_cert.pem',
    'DISTRIBUTOR_KEY_FILE' : certificatesPath + path.sep + 'distributor_active_cert/tizen_distributor_private_key.pem',
    'DISTRIBUTOR_CERT_FILE' : certificatesPath + path.sep + 'distributor_active_cert/tizen_distributor_cert.pem',
    'DISTRIBUTOR2_KEY_FILE': certificatesPath + path.sep + 'distributor_active_cert/tizen_distributor2_private_key.pem',
    'DISTRIBUTOR2_CERT_FILE': certificatesPath + path.sep + 'distributor_active_cert/tizen_distributor2_cert.pem'
};
...
var isSamsungCertificate = false;
...
function p12ToPem(p12Buffer, password, type, keyfile, certfile ) {
    logger.info(moduleName, '================Converting ' + type + ' P12 file to Pem file');
    var entry = parseCertificate(p12Buffer, password);
    if(!entry){
        logger.error(moduleName, 'Parse Certififate failed');
    ...
    if(entry.privateKey) {
        ...
        createDir(certificatesPath + path.sep + type + '_active_cert');
        fs.writeFileSync(keyfile, privateKeyP12Pem, 'utf8');
        logger.info(moduleName, 'Converted pem private key file is in ' + keyfile);
    } else {
        console.log('');
    }
    ...
    logger.info(moduleName, 'Converted pem cert file is in ' + certfile);
...
function getCertificateInfo(certPath, password){
    ...
    if(fs.existsSync(certPath)){
        ...
        logger.warning(moduleName, noCert);
...
function loadProfile(){
    ...
    if(fs.existsSync(profileFile)){
        logger.info(moduleName, 'Profile file path:'+profileFile);
        ...
        parseString(data,{ explicitArray : false}, function(err,result){
            ...
            {
                logger.info(moduleName, 'Active profile name:'+activeProfile);   
                ...
                if(typeof(profileItems) != 'undefined' && profileItems.length>2){
                    logger.info(moduleName, 'Find active profile:'+activeProfile);
                    
                    var tmpAuthorFile = extensionPath + profileItems[0].$.key;
                    ...
                    var tmpDistributorFile = extensionPath + profileItems[1].$.key;
                    ...
                    var tmpDistributorFile2 = extensionPath + profileItems[2].$.key;;
                    ...
                    if(fs.existsSync(authorFile) && fs.existsSync(distributorFile)){
                        logger.info(moduleName, 'Developer p12 File: '+authorFile);
                        logger.info(moduleName, 'Distributor p12 File: '+distributorFile);
                        profileFlag = true;
                    }

                    authorCa = profileItems[0].$.ca;
                    //distributorCa = profileItems[1].$.ca;
                    //distributorCa2 = profileItems[2].$.ca;
                    if (authorCa === '') {
                        console.log('This is Samsung Certificate!');
                        isSamsungCertificate = true;
                        certificateFilePath = path.dirname(authorFile);
                    }
                    else {
                        console.log('This is Tizen Certificate!');
                        isSamsungCertificate = false;
                    }
                }
            }
...
function checkActiveProfile() {
    logger.info(moduleName, '================Check Active Profile');
    ...
    if(flag){
        ...
        //logger.debug(moduleName, 'authorPassword:'+authorPassword);
        //logger.debug(moduleName,'distributorPassword:'+distributorPassword);

        var decodedAuthorPass = decryptPassword(authorPassword);
        var decodedDistributorPass = decryptPassword(distributorPassword);

        //logger.debug(moduleName,'decodedAuthorPass:'+decodedAuthorPass);
        //logger.debug(moduleName,'decodedDistributorPass:'+decodedDistributorPass);
...
function createDir(dirPath){
    if (!fs.existsSync(dirPath)){
        logger.info(moduleName, 'Create dir path:'+dirPath);
        try{        
        ...
        //logger.info(moduleName, dirPath+' is exist');
    }
}

...
function createKeyInfo(target, destfile) {
	...
	if (typeof(strCert) == 'undefined')
	{
		...
		logger.error(moduleName, noStrCert);
		common.showMsgOnWindow(common.ENUM_WINMSG_LEVEL.ERROR, noStrCert);
		return;
	}
...
function createSignatureXML(signatureId, destfile) {
	...
	if (typeof(privateKey) == 'undefined')
	{
		...
		logger.error(moduleName, noPrivateKey);
		common.showMsgOnWindow(common.ENUM_WINMSG_LEVEL.ERROR, noPrivateKey);
		return;
	}
...
function signPackage(dirPath,expli) {
	...
	if (workspacePath) {
		...
		logger.info(moduleName, 'Generated author-signature.xml and signature1.xml');
		var signSuccessMsg = 'Sign the package Successfully!';
		//common.showMsgOnWindow(common.ENUM_WINMSG_LEVEL.INFO, signSuccessMsg);
		logger.info(moduleName, signSuccessMsg);
